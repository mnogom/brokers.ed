...
async def request(socket: zmq.asyncio.Socket, session_id: bytes, message_id: int) -> bytes:
    message_id = str(message_id).encode()
    data = session_id + b" @ " + generate_data() + b" | " + message_id
    await socket.send(data)
    logger.info(f"Send request {data!r}")
    response = await socket.recv()
    if response[len(response) - len(data):] == data:
        logger.info(f"Receive response: {response!r} for session id: {session_id!r}, message id: {message_id!r}")
    else:
